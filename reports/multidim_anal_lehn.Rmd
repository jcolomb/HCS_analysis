...
```{r, evaluate =FALSE}
load("multidim_lehnardt_my88.rdata")
...
##tuning and performing svm  
  ...
  Accuracy =paste0(ncol(Input)-1," variables: Accuracy of the prediction with default kernel (Corrected Rand index: 0 denotes chance level, maximum is 1):",temp$crand)
  ...
  svm.model <- svm(groupingvar ~ ., data = trainset, cost = obj$best.parameters$cost, gamma = obj$best.parameters$gamma,kernel="linear")
  svm.pred <- predict(svm.model, testset %>% select(-groupingvar))
  ...
  #Accuracy of grouping and plot
  temp =classAgreement (SVMprediction_res)
  Accuracy =paste0(ncol(Input)-1," variables: Accuracy of the prediction with linear kernel (Corrected Rand index: 0 denotes chance level, maximum is 1):",temp$crand)
