####################################
# 5.1. plotting hourly mean around light off
####################################
...
#setwd(plot.path)
pdf(file = paste0(plot.path,"/14_Minutes_Behaviours_timedtolightoff.pdf"), width = 15, height = 10)

grid.arrange(splitTextGrob(text.all.d),pl.all.d,
             ncol=1,heights = c(1,5),
             top = textGrob(paste0('5. All behaviour kinetics with dark phase correction (',groupingby,' grouping)'),gp=gpar(fontsize = 30, font=3)))
